...
// 0x51C7E8
Object* dword_51C7E8 = NULL;

// 0x51C7EC
int dword_51C7EC = 0;

// 0x51C7F0
char* off_51C7F0 = byte_50D6B8;

// 0x51C7F4
char* off_51C7F4 = byte_50D6C0;
...
{
    sprintf(byte_66772C, "%d:%02d", (gGameTime / 600) / 60 % 24, (gGameTime / 600) % 60);
    return byte_66772C;
}
    ...
    if (gameGetGlobalVar(GVAR_ENEMY_ARROYO)) {
        ...
        if (day >= 360 && gameGetGlobalVar(GVAR_FALLOUT_2) != 3) {
            if (!gameMovieIsSeen(MOVIE_ARTIMER4)) {
                ...
                sub_4C46CC(CITY_ARROYO, 0, 1);
                sub_4C46CC(CITY_DESTROYED_ARROYO, 1, 1);
                sub_4C4634(CITY_DESTROYED_ARROYO, 2);
            }
...
int mapUpdateEventProcess(Object* obj, void* data)
{
    ...
    sub_4A2790(EVENT_TYPE_MAP_UPDATE_EVENT, NULL);
...
void sub_4A3CA0()
{
    if (!sub_444D2C() && !isInCombat()) {
        ...
        dword_51C7DC += 1;
        if (dword_51C7DC >= scriptsCount) {
            dword_51C7DC = 0;
        }

        if (dword_51C7DC < scriptsCount) {
            int proc = isInCombat() ? SCRIPT_PROC_COMBAT : SCRIPT_PROC_CRITTER;
            int extentIndex = dword_51C7DC / SCRIPT_LIST_EXTENT_SIZE;
            int scriptIndex = dword_51C7DC % SCRIPT_LIST_EXTENT_SIZE;
...
void sub_4A3D84()
{
    ...
    if (getTicksBetween(v0, dword_51C7E0) >= 100) {
        dword_51C7E0 = v0;
        if (!isInCombat()) {
            gGameTime += 1;
        }
        ...
        if (elevatorSelectLevel(gScriptsRequestedElevatorType, &map, &elevation, &tile) != -1) {
            ...
            if (map == gMapHeader.field_34) {
                if (elevation == gElevation) {
                    ...
                    sub_49D4D4(gDude, tile, elevation, 0);
                } else {
...
//
// 0x4A45D4
void sub_4A45D4(STRUCT_664980* a1)
{
    if (a1 != NULL) {
        memcpy(&stru_664958, a1, sizeof(stru_664958));
    } else {
...
int scriptExecProc(int sid, int proc)
{
    ...
    if (programLoaded) {
        ...
        sub_46CCA4(program, -1);
    }

    script->action = proc;

    sub_46DD2C(program, v9);
...
int scriptsInit()
{
    ...
    for (int index = 0; index < 1450; index++) {
        if (!messageListInit(&(stru_6649D4[index]))) {
            return -1;
        }
    }

    sub_4A63E0();
    sub_4670FC(sub_4DC30C);
...
int sub_4A5138()
{
    ...
    for (i = 0; i < 1450; i++) {
        if (!messageListInit(&(stru_6649D4[i]))) {
...
int sub_4A63E0()
{
    sub_4A2790(EVENT_TYPE_SCRIPT, NULL);
    sub_4A52F4();
...
int scriptsGetMessageList(int a1, MessageList** messageListPtr)
{
    ...
    MessageList* messageList = &(stru_6649D4[messageListIndex]);
    if (messageList->entries_num == 0) {
        char scriptName[20];
        scriptName[0] = '\0';
...
{
    return sub_4A6C5C(messageListId, messageId, 0);
}
...
int scriptGetLocalVar(int sid, int variable, int* value)
{
    if (sid >> 24 == SCRIPT_TYPE_SYSTEM) {
        ...
        byte_667750[0] = '\0';
        scriptsGetFileName(sid & 0xFFFFFF, byte_667750);
    ...
    if (script->localVarsCount == 0) {
        // NOTE: Uninline.
        sub_4A4F28(script->field_14, &(script->field_50), sid);
    }

    if (script->localVarsCount > 0) {
        if (script->localVarsOffset == -1) {
            script->localVarsOffset = sub_4822E0(script->localVarsCount);
        }
