...
// 0x51D478
STRUCT_51D478* off_51D478 = NULL;

// 0x51D47C
STRUCT_51D478* off_51D47C = NULL;

// 0x51D480
unsigned int dword_51D480 = UINT_MAX;
...
// 0x51D4B4
char* (*off_51D4B4)(char* path) = sub_4AC78C;
...
void soundExit()
{
    ...
    if (dword_51D480 != -1) {
        removeTimedEvent(&dword_51D480);
    }

    while (off_51D47C != NULL) {
        STRUCT_51D478* next = off_51D47C->next;
        gSoundFreeProc(off_51D47C);
        off_51D47C = next;
    }
...
Sound* soundAllocate(int a1, int a2)
{
    ...
    wfxFormat->nSamplesPerSec = dword_66815C;
    ...
    sound->field_78 = dword_66816C;
    sound->readLimit = sound->field_7C * dword_66816C;
...
int soundLoad(Sound* sound, char* filePath)
{
    ...
    sound->io.fd = sound->io.open(off_51D4B4(filePath), 0x0200);
    if (sound->io.fd == -1) {
...
// 0x4AD5C8
int addSoundData(Sound* sound, unsigned char* buf, int size)
{
    HRESULT hr;
    void* audio_ptr_1;
    DWORD audio_bytes_1;
...
int soundSetVolume(Sound* sound, int volume)
{
    ...
    normalizedVolume = soundVolumeHMItoDirectSound(dword_668154 * volume / VOLUME_MAX);
...
int soundGetVolume(Sound* sound)
{
    ...
    v8 = VOLUME_MAX * v13 / dword_668154;
...
int soundSetChannels(Sound* sound, int channels)
{
    ...
    if (channels == 3) {
        ...
        format->nChannels = 2;
        format->nAvgBytesPerSec = format->nBlockAlign * dword_66815C;
    }
...
// 0x4AE830
void removeFadeSound(STRUCT_51D478* a1)
{
    STRUCT_51D478* prev;
    STRUCT_51D478* next;
    STRUCT_51D478* tmp;
...
void fadeSounds()
{
    ...
    if (off_51D478 == NULL && dword_51D480 != -1) {
        timeKillEvent(dword_51D480);
        dword_51D480 = -1;
    }
}

// 0x4AE988
int internalSoundFade(Sound* sound, int a2, int a3, int a4)
{
    STRUCT_51D478* ptr;
    ...
    ptr->field_8 = a3;
    ptr->field_C = soundGetVolume(sound);
    ...
    dword_51D480 = timeSetEvent(40, 10, sub_4AE5C8, (DWORD_PTR)fadeSounds, 1);
    if (dword_51D480 == 0) {
        gSoundLastError = SOUND_UNKNOWN_ERROR;
        return gSoundLastError;
    }
...
// 0x4AEB0C
int soundFade(Sound* sound, int a2, int a3)
{
    return internalSoundFade(sound, a2, a3, 0);
}
