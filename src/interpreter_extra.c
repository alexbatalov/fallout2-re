...
// 0x45409C
int sub_45409C(Object* a1, Object* a2, int flags)
{
    if (a1 == gDude) {
        bool v1 = true;
        if (gameUiIsDisabled()) {
...
void opMarkAreaKnown(Program* program)
{
    ...
    if (data[2] == 0) {
        if (data[0] == CITY_STATE_INVISIBLE) {
            sub_4C46CC(data[1], 0, 1);
        } else {
            sub_4C46CC(data[1], 1, 1);
            sub_4C4634(data[1], data[0]);
        }
    } else if (data[2] == 1) {
        sub_4BFB08(data[1]);
    }
...
void opCreateObject(Program* program)
{
    ...
    if (sid != -1) {
        ...
        script->owner = object;
        sub_4A4F28(sid - 1, &(script->field_50), object->sid);
    };
...
void opDestroyObject(Program* program)
{
    ...
    if (owner != NULL) {
        int quantity = sub_47808C(owner, object);
...
void opDrop(Program* program)
{
    ...
    sub_49B8B0(script->target, object);
}
...
void opRemoveObjectFromInventory(Program* program)
{
    ...
    if (itemRemove(owner, item, 1) == 0) {
        Rect rect;
        sub_489EC4(item, 1, 0, &rect);
        tileWindowRefreshRect(&rect, item->elevation);

        if (updateFlags) {
            sub_45409C(owner, item, flags);
        }
    }
}
...
void opWieldItem(Program* program)
{
    ...
    if (sub_472758(critter, item, hand) == -1) {
        scriptPredefinedError(program, "wield_obj_critter", SCRIPT_ERROR_FOLLOWS);
    ...
    if (critter == gDude) {
        if (v1) {
            sub_4715F8(critter, oldArmor, newArmor);
        }
...
void opUseObject(Program* program)
{
    ...
    if ((self->pid >> 24) == OBJ_TYPE_CRITTER) {
        sub_412114(script->target, object);
    } else {
        sub_49C740(self, object);
    }
}
    ...
    if (object1 != NULL && object2 != NULL) {
        if (object2->tile != -1) {
            ...
            if (objectCanHearObject(object1, object2)) {
                Object* a5;
                sub_4163AC(object1, object1->tile, object2->tile, NULL, &a5, 16);
                if (a5 == object2) {
                    canSee = true;
                }
            }
...
void opStartGameDialog(Program* program)
{
    ...
    sub_44510C(gGameDialogHeadFid, gGameDialogReactionOrFidget);
}
...
void opMetarule3(Program* program)
{
    ...
    case METARULE3_CLR_FIXED_TIMED_EVENTS:
        if (1) {
            sub_4A3E30((Object*)data[2], data[1]);
            sub_4A2790(EVENT_TYPE_SCRIPT, sub_4A3E3C);
        }
        break;
    case METARULE3_MARK_SUBTILE:
        result = sub_4C35A8(data[2], data[1], data[0]);
        break;
    case METARULE3_GET_KILL_COUNT:
        result = killsGetByType(data[2]);
        break;
    case METARULE3_MARK_MAP_ENTRANCE:
        result = sub_4BFD50(data[2], data[1], data[0]);
        break;
    case METARULE3_WM_SUBTILE_STATE:
        if (1) {
            int state;
            if (sub_4C3740(data[2], data[1], &state) == 0) {
                result = state;
            }
        }
        break;
...
int sub_457BF4(Object* critter, int anim, bool forceBack)
{
    ...
    while (obj != NULL) {
        ...
        if ((obj->flags & 0x01) == 0 && obj->pid == pid && !critterIsDead(obj)) {
            ...
            if (deathFrame != 0) {
                sub_426DDC(obj);
                if (deathFrame == 1) {
                    int anim = sub_457BF4(obj, dword_518ED0[v3], 1);
...
void opCritterDamage(Program* program)
{
    ...
    sub_413494(self->tile, self->elevation, amount, amount, damageType, animate, bypassArmor);
...
void opCritterAttemptPlacement(Program* program)
{
    ...
    int rc = sub_49D4D4(critter, tile, elevation, 1);
    programStackPushInt32(program, rc);
    programStackPushInt16(program, VALUE_TYPE_INT);
}
...
void opGetProtoData(Program* program)
{
    ...
    int value = 0;
    int valueType = sub_49FFD8(pid, member, &value);
    switch (valueType) {
...
void opGetMessageString(Program* program)
{
    ...
    char* string;
    if (messageIndex >= 1) {
        string = sub_4A6C5C(messageListIndex, messageIndex, 1);
        if (string == NULL) {
...
// inven_cmds
// 0x459178
void sub_459178(Program* program)
{
    opcode_t opcode[3];
    int data[3];
...
void opMetarule(Program* program)
{
    ...
    case METARULE_DROP_ALL_INVEN:
        if (1) {
            ...
            result = sub_477804(object, object->tile);
            if (gDude == object) {
    ...
    case METARULE_GET_WORLDMAP_XPOS:
        sub_4C47A4(&result, NULL);
        break;
    case METARULE_GET_WORLDMAP_YPOS:
        sub_4C47A4(NULL, &result);
        break;
    case METARULE_CURRENT_TOWN:
        if (sub_4C47C0(&result) == -1) {
...
void opRemoveMultipleObjectsFromInventory(Program* program)
{
    ...
    int quantity = sub_47808C(owner, item);
    ...
    if (quantity != 0) {
        if (itemRemove(owner, item, quantity) == 0) {
            Rect updatedRect;
            sub_489EC4(item, 1, 0, &updatedRect);
            if (itemWasEquipped) {
                if (owner == gDude) {
                    bool v1 = true;
                    if (gameUiIsDisabled()) {
...
// gsay_start
// 0x45A56C
void sub_45A56C(Program* program)
{
    program->flags |= PROGRAM_FLAG_0x20;
...
// gsay_end
// 0x45A5B0
void sub_45A5B0(Program* program)
{
    program->flags |= PROGRAM_FLAG_0x20;
    sub_4456D8();
    program->flags &= ~PROGRAM_FLAG_0x20;
...
// gsay_reply
// 0x45A5D4
void sub_45A5D4(Program* program)
{
    program->flags |= PROGRAM_FLAG_0x20;
...
// gsay_option
// 0x45A6C4
void sub_45A6C4(Program* program)
{
    program->flags |= PROGRAM_FLAG_0x20;
...
// gsay_message
// 0x45A8AC
void sub_45A8AC(Program* program)
{
    program->flags |= PROGRAM_FLAG_0x20;
...
// giq_option
// 0x45A9B4
void sub_45A9B4(Program* program)
{
    program->flags |= PROGRAM_FLAG_0x20;
...
// gdialog_barter
// 0x45AFA0
void sub_45AFA0(Program* program)
{
    opcode_t opcode = programStackPopInt16(program);
    int data = programStackPopInt32(program);
...
// inven_unwield
void sub_45B0A8(Program* program)
{
    Object* obj;
    int v1;
...
// anim_action_frame
void sub_45B64C(Program* program)
{
    opcode_t opcode[2];
    int data[2];
...
void opDestroyMultipleObjects(Program* program)
{
    ...
    Object* owner = objectGetOwner(object);
    if (owner != NULL) {
        int quantityToDestroy = sub_47808C(owner, object);
...
void opUseObjectOnObject(Program* program)
{
    ...
    Object* self = scriptGetSelf(program);
    if ((self->pid >> 24) == OBJ_TYPE_CRITTER) {
        sub_411F2C(self, target, item);
    } else {
        sub_49C5FC(self, target, item);
    }
}
...
void opMoveObjectInventoryToObject(Program* program)
{
    ...
    if (object1 != gDude && item2 != NULL) {
        ...
        sub_45409C(object1, item2, flags);
    }

    sub_4776AC(object1, object2);

    if (object1 == gDude) {
        if (oldArmor != NULL) {
            sub_4715F8(gDude, oldArmor, NULL);
        }
...
void sub_45CDD8()
{
    ...
    interpreterRegisterOpcode(0x8109, sub_459178); // op_inven_cmds
    ...
    interpreterRegisterOpcode(0x811C, sub_45A56C);
    interpreterRegisterOpcode(0x811D, sub_45A5B0);
    interpreterRegisterOpcode(0x811E, sub_45A5D4); // op_gsay_reply
    interpreterRegisterOpcode(0x811F, sub_45A6C4); // op_gsay_option
    interpreterRegisterOpcode(0x8120, sub_45A8AC); // op_gsay_message
    interpreterRegisterOpcode(0x8121, sub_45A9B4); // op_giq_option
    ...
    interpreterRegisterOpcode(0x8129, sub_45AFA0); // op_gdialog_barter
    ...
    interpreterRegisterOpcode(0x812C, sub_45B0A8); // op_inven_unwield
    ...
    interpreterRegisterOpcode(0x813A, sub_45B64C); // op_anim_action_frame
