...
// NOTE: Uncollapsed 0x4C614C. The opposite of [sub_4C6244]. It can be either
// signed vs. unsigned variant, as well as int vs. long. It's provided here to
// identify places where data was written with [sub_4C6244].
...
{
    return sub_4C614C(stream, (int*)valuePtr);
}
...
{
    // NOTE: Uninline.
    return sub_4C6244(stream, value);
}
...
{
    return sub_4C6244(stream, (int)value);
}
...
{
    return sub_4C6244(stream, value ? 1 : 0);
}
...
// NOTE: Uncollapsed 0x4C63BC. The opposite of [sub_4C6550].
...
int fileWriteInt32List(File* stream, int* arr, int count)
{
    for (int index = 0; index < count; index++) {
        // NOTE: Uninline.
        if (sub_4C6244(stream, arr[index]) == -1) {
            return -1;
        }
    }
...
int fileNameListInit(const char* pattern, char*** fileNameListPtr, int a3, int a4)
{
    ...
    if (!sub_4DFF28(pattern, fileList)) {
        free(fileList);
        return 0;
    }
    ...
    if (fileList->fileNamesLength != 0) {
        qsort(fileList->fileNames, fileList->fileNamesLength, sizeof(*fileList->fileNames), sub_4C68E8);
